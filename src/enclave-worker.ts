...
export class EnclaveWorker {
  ...
  async calculateHistoricalReturns(
    ...
    try {
      ...
      for (const ex of exchanges) {
        // This will internally read trades and calculate returns
        // But only return aggregated results
        const returns = await this.equitySnapshotAggregator.calculateHistoricalReturns(
          userUid,
          ex,
          startDate,
          endDate
        ...
        // Convert to safe format (no individual trade details)
        for (const r of returns) {
          allReturns.push({
            period: `${r.periodStart.toISOString()}-${r.periodEnd.toISOString()}`,
            netReturn: r.netReturn,
            percentageReturn: r.percentageReturn,
