...
app.get('/api/enclave-logs/stream', async (req, res) => {
  ...
  const connectToStream = async () => {
    try {
      ...
      const response = await fetch(ENCLAVE_SSE_URL, { signal: controller.signal });
      ...
      const errorCode = (error as { cause?: { code?: string } })?.cause?.code || (error as { code?: string })?.code;
      // Only log once, not on every retry or timeout
      if (retryCount === 0 && errorCode !== 'UND_ERR_BODY_TIMEOUT' && errorCode !== 'ABORT_ERR') {
        console.warn('[Proxy] Enclave SSE stream unavailable');
      }

      if (retryCount < maxRetries && !req.closed) {
        retryCount++;
...
// Get Enclave Public Key endpoint (not available in local dev mode)
app.get('/api/public-key', (_req, res) => {
  const timestamp = new Date().toLocaleTimeString();
  addLogToBuffer(`[${timestamp}] → GetEnclavePublicKey (local dev mode - mock response)`);

  // In local dev mode, attestation is skipped - return mock response
  res.json({
    public_key: 'LOCAL_DEV_MODE_NO_ATTESTATION',
    public_key_hash: 'local-dev-' + crypto.randomBytes(16).toString('hex'),
    algorithm: 'MOCK_RSA_2048',
    note: 'Attestation not available in local development mode (SKIP_ATTESTATION=true)'
  });
});

// Get Attestation Report endpoint (not available in local dev mode)
app.post('/api/attestation', (req, res) => {
  const timestamp = new Date().toLocaleTimeString();
  const nonce = req.body.nonce || Buffer.from(crypto.randomBytes(32)).toString('base64');
  addLogToBuffer(`[${timestamp}] → GetAttestationReport (local dev mode - mock response)`);
...
<body>
  <div class="layout">
    ...
    <aside class="sidebar">
      ...
      <div class="config-section">
        ...
        <button class="btn btn-secondary" onclick="getPublicKey()">
          Get Public Key
        </button>
        <button class="btn btn-secondary" onclick="getAttestation()">
          Get Attestation
        </button>
        <button class="btn btn-secondary" onclick="verifyMetrics()">
          Verify Metrics Signature
        </button>
      </div>
    ...
    <main class="main">
      ...
      <div class="breakdown-grid" id="breakdown-section" style="display: none;">
        ...
        <div class="breakdown-card" id="global-card">
          ...
          <div class="breakdown-stats">
            <div class="breakdown-stat">
              ...
              <span class="value" id="global-equity">--</span>
            </div>
            <div class="breakdown-stat">
        ...
        <div class="breakdown-card crypto-card" id="spot-card" style="display: none;">
          ...
          <div class="breakdown-stats">
            <div class="breakdown-stat"><span class="label">Equity</span><span class="value" id="spot-equity">--</span></div>
        ...
        <div class="breakdown-card crypto-card" id="swap-card" style="display: none;">
          ...
          <div class="breakdown-stats">
            <div class="breakdown-stat"><span class="label">Equity</span><span class="value" id="swap-equity">--</span></div>
        ...
        <!-- Crypto: Earn (Staking/Lending/Savings) -->
        <div class="breakdown-card crypto-card" id="earn-card" style="display: none;">
          <div class="breakdown-header">
            <span class="market-name">Earn</span>
          </div>
          <div class="breakdown-stats">
            <div class="breakdown-stat"><span class="label">Equity</span><span class="value" id="earn-equity">--</span></div>
            <div class="breakdown-stat"><span class="label">APY Rewards</span><span class="value" id="earn-rewards">--</span></div>
          </div>
        </div>
        <!-- IBKR: Stocks (no equity breakdown available from IBKR) -->
        ...
        <!-- IBKR: Futures & Commodities (FUT + CMDTY merged) -->
        <div class="breakdown-card ibkr-card" id="futures_commodities-card" style="display: none;">
          <div class="breakdown-header">
            <span class="market-name">Futures & Commodities</span>
          </div>
          <div class="breakdown-stats">
            <div class="breakdown-stat"><span class="label">Volume</span><span class="value" id="futures_commodities-volume">--</span></div>
            <div class="breakdown-stat"><span class="label">Trades</span><span class="value" id="futures_commodities-trades">--</span></div>
            <div class="breakdown-stat"><span class="label">Trading Fees</span><span class="value" id="futures_commodities-trading-fees">--</span></div>
          </div>
        </div>
        <!-- IBKR: Forex -->
        ...
        <!-- Shared: Options (no equity breakdown) -->
  ...
  <script>
    ...
    function updateBreakdown(snapshots) {
      // Categories: crypto = spot/swap/earn, traditional = stocks/cfd/futures_commodities/forex
      const categories = ['global', 'spot', 'swap', 'earn', 'stocks', 'cfd', 'futures_commodities', 'forex', 'options'];
      const totals = {};
      ...
      // Sort snapshots by timestamp (newest first) to get latest equity values
      ...
      // Get equity from LATEST snapshot only (point-in-time metric)
      const latestSnapshot = sortedSnapshots[0];
      if (latestSnapshot?.breakdown) {
        hasBreakdown = true;
        categories.forEach(cat => {
          if (latestSnapshot.breakdown[cat]) {
            totals[cat].equity = parseFloat(latestSnapshot.breakdown[cat].equity || 0);
          }
        });
      }

      // Sum volume/trades/fees across ALL snapshots (cumulative metrics)
      snapshots.forEach(snapshot => {
        ...
        // Backward compatibility: merge old 'futures' and 'commodities' into 'futures_commodities'
        if (b.futures) {
          totals.futures_commodities.volume += parseFloat(b.futures.volume || 0);
          totals.futures_commodities.trades += parseInt(b.futures.trades || 0);
          totals.futures_commodities.trading_fees += parseFloat(b.futures.trading_fees || 0);
        }
        if (b.commodities) {
          totals.futures_commodities.volume += parseFloat(b.commodities.volume || 0);
          totals.futures_commodities.trades += parseInt(b.commodities.trades || 0);
          totals.futures_commodities.trading_fees += parseFloat(b.commodities.trading_fees || 0);
        }
      });
      ...
      document.getElementById('global-equity').textContent = '$' + formatNumber(totals.global.equity);
      ...
      // Crypto categories (with equity per market)
      updateCategoryCard('spot', true);
      updateCategoryCard('swap', true);
      updateCategoryCard('earn', true);

      // IBKR/Traditional categories (no equity breakdown from IBKR, only volume/trades/fees)
      updateCategoryCard('stocks', false);
      updateCategoryCard('cfd', false);
      updateCategoryCard('futures_commodities', false);
      updateCategoryCard('forex', false);
      updateCategoryCard('options', false);
    }
    ...
    // Enclave public key state
    let enclavePublicKey = null;
    let enclavePublicKeyHash = null;

    // Get enclave public key
    async function getPublicKey() {
      try {
        addLog('info', 'Fetching enclave public key...');
        const response = await fetch('/api/public-key');
        const data = await response.json();

        if (data.error) {
          addLog('error', 'Failed to get public key: ' + data.error);
          alert('Error: ' + data.error);
          return;
        }

        enclavePublicKey = data.public_key;
        enclavePublicKeyHash = data.public_key_hash;

        addLog('info', 'Public Key received - Algorithm: ' + data.algorithm);
        addLog('info', 'Hash: ' + data.public_key_hash);
        addLog('info', 'Generated at: ' + data.generated_at);

        alert('Public Key Retrieved!\\n\\nAlgorithm: ' + data.algorithm + '\\nHash: ' + data.public_key_hash.substring(0, 32) + '...\\n\\nKey stored for signature verification.');
    ...
    async function getAttestation() {
      try {
        addLog('info', 'Requesting attestation report...');
        ...
        if (data.error) {
          addLog('error', 'Attestation failed: ' + data.error);
          alert('Attestation Error: ' + data.error);
          return;
        }

        const verified = data.verified ? 'YES' : 'NO';
        const sevSnp = data.sev_snp_enabled ? 'YES' : 'NO (Development Mode)';

        addLog('info', 'Attestation Report received');
        addLog('info', 'Verified: ' + verified + ', SEV-SNP: ' + sevSnp);

        if (data.measurement) {
          addLog('info', 'Measurement: ' + data.measurement.substring(0, 32) + '...');
        }

        let msg = 'Attestation Report\\n\\n';
        msg += 'Verified: ' + verified + '\\n';
        msg += 'SEV-SNP Enabled: ' + sevSnp + '\\n';
        if (data.measurement) {
          msg += 'Measurement: ' + data.measurement.substring(0, 32) + '...\\n';
        }
        if (data.platform_version) {
          msg += 'Platform: ' + data.platform_version + '\\n';
        }
        if (data.error) {
          msg += '\\nNote: ' + data.error;
        }
    ...
    async function verifyMetrics() {
      if (!enclavePublicKey) {
        alert('Please get the enclave public key first!');
        return;
      }
      ...
      try {
        addLog('info', 'Fetching signed metrics...');
        ...
        if (data.error) {
          addLog('error', 'Failed to get metrics: ' + data.error);
          alert('Error: ' + data.error);
          return;
        }

        // Check public key hash
        if (data.public_key_hash !== enclavePublicKeyHash) {
          addLog('error', 'PUBLIC KEY MISMATCH! Possible tampering detected!');
          alert('SECURITY WARNING!\\n\\nPublic key hash mismatch!\\nExpected: ' + enclavePublicKeyHash.substring(0, 16) + '...\\nReceived: ' + data.public_key_hash.substring(0, 16) + '...');
          return;
        }

        addLog('info', 'Public key hash matches');
        addLog('info', 'Signature: ' + data.signature.substring(0, 32) + '...');

        // Parse signed_at (comes as string from gRPC int64)
        const signedAtMs = typeof data.signed_at === 'string' ? parseInt(data.signed_at) : data.signed_at;
        const signedAtDate = new Date(signedAtMs);
        addLog('info', 'Signed at: ' + signedAtDate.toISOString());

        // Note: Full cryptographic verification would require Web Crypto API
        // For demo, we show the signature data
        let msg = 'Metrics Signature Verification\\n\\n';
        msg += 'Public Key Hash: MATCH\\n';
        msg += 'Signature: ' + data.signature.substring(0, 32) + '...\\n';
        msg += 'Signed at: ' + signedAtDate.toISOString() + '\\n\\n';
        msg += 'Metrics:\\n';
        msg += '- Total Equity: $' + parseFloat(data.total_equity || 0).toFixed(2) + '\\n';
        msg += '- Total Balance: $' + parseFloat(data.total_balance || 0).toFixed(2) + '\\n';
        msg += '- Total Trades: ' + (data.total_trades || 0);
